...
-- P1-1 命令注入防护：对拼入单引号 shell 的字符串做安全转义。
-- OpenWrt /bin/sh 标准：将 ' 替换为 '\'' ，使任意输入（含引号/分号/换行）都留在单个引号字符串字面量内。
local function shell_quote(s)
    s = tostring(s or "")
    return (s:gsub("'", "'\\''"))
end

-- P1-1 命令注入防护：存储路径白名单校验。
-- 仅允许安全字符（字母数字、_、/、.、-、空格），拒绝一切 shell 元字符（' ; $ ` & | < > 换行等）。
-- 返回 (是否合法, 规范化路径)。拒绝时 path 置空，避免进入 shell 拼接。
local safe_path_pat = "^[A-Za-z0-9_/.-]+$"
local function sanitize_storage_path(path)
    if not path or path == "" then return false, "" end
    if not path:match(safe_path_pat) then return false, "" end
    return true, path
end
...
local function migrate_storage(new_path)
    ...
    -- P1-1 命令注入防护：双保险——入口先做白名单校验，非法即拒绝，不进入任何 shell 拼接。
    local ok_path, plain_path = sanitize_storage_path(new_path)
    if not ok_path then
        return false, "路径含非法字符，已拒绝迁移"
    end
    new_path = plain_path
    local qp = shell_quote(new_path) -- 单引号安全转义后用于 shell 拼接
    ...
    if mount_base then
        local stat = util.trim(util.exec("df '" .. shell_quote(mount_base) .. "' 2>/dev/null | tail -1 | awk '{print $2}'"))
        if stat == "" or stat == "0" then
            return false, "外部存储 " .. mount_base .. " 不可用或未挂载"
        end
        -- 可写性测试：创建目标目录并尝试写入一个临时文件（替代原先放在下拉枚举处的 touch 测试）
        sys.call("mkdir -p '" .. qp .. "'")
        local wt = util.trim(util.exec("touch '" .. qp .. "/.mg_wtest' 2>/dev/null && echo 1; rm -f '" .. qp .. "/.mg_wtest' 2>/dev/null"))
        if wt ~= "1" then
    ...
    -- 1. 创建目标目录
    sys.call("mkdir -p '" .. qp .. "/data'")

    -- 2. 复制配置文件（/etc/config/model-gateway 始终是普通文件，uci 依赖它）
    sys.call("rm -f '" .. qp .. "/model-gateway'")
    sys.call("cp /etc/config/model-gateway '" .. qp .. "/model-gateway'")

    -- 验证目标配置文件已创建
    local target_ok = util.exec("test -s '" .. qp .. "/model-gateway' && echo 1 || echo 0")
    ...
    if stat_data == "1" then
        sys.call("cp -a '" .. default_data .. "/.' '" .. qp .. "/data/' 2>/dev/null")
    end
...
local function sync_config_to_external(config_path)
    ...
    -- P1-1 命令注入防护：同步路径同样做白名单校验 + shell 转义。
    local okp, cp_plain = sanitize_storage_path(config_path)
    if not okp then return end
    local qc = shell_quote(cp_plain)
    sys.call("test -d '" .. qc .. "' && cp /etc/config/model-gateway '" .. qc .. "/model-gateway' 2>/dev/null")
end
...
o.cfgvalue = function(self, section)
    ...
    if cp and cp ~= "" then
        ...
        local exists = (sys.call("test -s '" .. shell_quote(cp) .. "/model-gateway' >/dev/null 2>&1") == 0)
        if exists then
...
-- P1-1 命令注入防护：自由路径必须通过白名单校验，拒绝含引号/分号/换行等 shell 元字符的输入。
o.validate = function(self, value)
    if value == "" then return true end
    local ok = value:match(safe_path_pat)
    if not ok then
        return nil, translate("路径含非法字符，仅允许字母/数字/下划线/点/斜杠/横线")
    end
    return true
end
...
m.on_after_commit = function(self)
    ...
    -- P1-1: selector/custom 为可能含注入的用户输入，日志内存放经转义与去换行的安全串
    sys.call("logger -t model-gateway 'on_after_commit: selector=" .. shell_quote(selector):gsub("\n", " ") .. " custom=" .. shell_quote(custom):gsub("\n", " ") .. "'")
    ...
    if new_path ~= "" then
        ...
        if ok then
            ...
            sys.call("logger -t model-gateway '路径迁移成功: " .. shell_quote(new_path):gsub("\n", " ") .. "'")
        else
            sys.call("logger -t model-gateway '路径迁移失败: " .. shell_quote(tostring(msg)):gsub("\n", " ") .. "'")
        end
    else
        -- 切回默认路径
        local current_cp_val = get_config_path()
    ...
    if new_path ~= "" and ok then
        -- P1-1: 写 UCI 的路径再次 sanitize 兜底（即便 migrate 已校验），杜绝任何注入残留
        local okp, np = sanitize_storage_path(new_path)
        if okp then
            table.insert(batch_cmds, "set model-gateway.settings.config_path='" .. np .. "'")
        else
            ok = false
            msg = "路径含非法字符，未写入 config_path"
        end
    ...
    if final_cp ~= "" and ok and new_path ~= "" then
        ...
        sys.call("logger -t model-gateway '配置已同步到外部: " .. shell_quote(final_cp):gsub("\n", " ") .. "'")
    end

    -- 界面反馈：写入 last_path_msg（页面顶部消息框 self.message + 持久化字段“上次路径操作”均可见）
    self.message = msg
    sys.call("uci set model-gateway.settings.last_path_msg='" .. shell_quote(msg):gsub("\n", " ") .. "' 2>/dev/null")
